var poc = function () {	
	...
	return {
		...
		basket : new Basket(),
		// this needs an xml request to load the feeds as saved in an account
		init : function(doc){			
			adaptor.initjQuery();
			sqlite.initDatabase(function(){
				...
				sqlite.insertEgg("reddit", 
                          "http://www.reddit.com/.rss",
			              utility.method.rss,
			              utility.service.other);
				sqlite.insertEgg("gizmodo", 
		...
		run : function (doc) {	
			...
			chrome.extension.sendRequest({"action" : "test1"}, function(test){
				eval(test);
				Test();
			});
